...
function findContainingBuilding(x,y){for(const b of WORLD.buildings||[]){if(pointInsideBuildingInterior(x,y,b))return b;}for(const b of WORLD.buildings||[]){if(pointInRect(x,y,b))return b;}return null;}
function flashlightInteriorRect(cam,b,pad=1){const x=Math.round(b.x-cam.x+pad),y=Math.round(b.y-cam.y+pad),w=Math.max(0,Math.round(b.w-pad*2)),h=Math.max(0,Math.round(b.h-pad*2));return {x,y,w,h};}
function beginFlashlightBuildingClip(maskCtx,cam,b,pad=1){if(!b)return false;const r=flashlightInteriorRect(cam,b,pad);if(r.w<=0||r.h<=0)return false;maskCtx.save();maskCtx.beginPath();maskCtx.rect(r.x,r.y,r.w,r.h);maskCtx.clip();return true;}
function getBuildingSideSegment(b,side){if(side==='left')return[{x:b.x,y:b.y},{x:b.x,y:b.y+b.h}];if(side==='right')return[{x:b.x+b.w,y:b.y},{x:b.x+b.w,y:b.y+b.h}];if(side==='top')return[{x:b.x,y:b.y},{x:b.x+b.w,y:b.y}];return[{x:b.x,y:b.y+b.h},{x:b.x+b.w,y:b.y+b.h}];}
function beamHitsBuildingSide(sourceWorldX,sourceWorldY,beamAng,halfAngle,b,side,pad=0.14){const seg=getBuildingSideSegment(b,side);const pts=[seg[0],seg[1],{x:(seg[0].x+seg[1].x)*0.5,y:(seg[0].y+seg[1].y)*0.5}];let minRel=Infinity,maxRel=-Infinity;for(const pt of pts){const rel=angleDiff(Math.atan2(pt.y-sourceWorldY,pt.x-sourceWorldX),beamAng);if(Math.abs(rel)<=halfAngle+pad)return true;minRel=Math.min(minRel,rel);maxRel=Math.max(maxRel,rel);}return minRel<=halfAngle+pad&&maxRel>=-(halfAngle+pad);}
function getBuildingVisibleSides(sourceWorldX,sourceWorldY,b){const sides=[];const eps=1;if(sourceWorldX<=b.x+eps)sides.push('left');else if(sourceWorldX>=b.x+b.w-eps)sides.push('right');if(sourceWorldY<=b.y+eps)sides.push('top');else if(sourceWorldY>=b.y+b.h-eps)sides.push('bottom');return sides;}
function getBuildingShadowEdges(sourceWorldX,sourceWorldY,beamAng,halfAngle,b){const allSides=['left','right','top','bottom'];const visible=getBuildingVisibleSides(sourceWorldX,sourceWorldY,b);const candidate=visible.length?visible:allSides;const litSides=[];for(const side of candidate){if(beamHitsBuildingSide(sourceWorldX,sourceWorldY,beamAng,halfAngle,b,side))litSides.push(side);}
if(!litSides.length&&candidate.length){let best=candidate[0],bestDiff=Infinity;for(const side of candidate){const seg=getBuildingSideSegment(b,side);const c={x:(seg[0].x+seg[1].x)*0.5,y:(seg[0].y+seg[1].y)*0.5};const d=Math.abs(angleDiff(Math.atan2(c.y-sourceWorldY,c.x-sourceWorldX),beamAng));if(d<bestDiff){bestDiff=d;best=side;}}litSides.push(best);}
const lit=new Set(litSides);const edges=[];for(const side of allSides){if(lit.has(side))continue;edges.push(getBuildingSideSegment(b,side));}return edges;}
function fillBuildingProjectedShadow(tctx,cam,sourceWorldX,sourceWorldY,beamAng,halfAngle,b,baseLen){const edges=getBuildingShadowEdges(sourceWorldX,sourceWorldY,beamAng,halfAngle,b);const sourceSx=Math.round(sourceWorldX-cam.x),sourceSy=Math.round(sourceWorldY-cam.y);const cx=b.x+b.w*0.5,cy=b.y+b.h*0.5,dist=Math.hypot(cx-sourceWorldX,cy-sourceWorldY),rectR=Math.hypot(b.w*0.5,b.h*0.5);for(const edge of edges){const startA=worldToScreen(edge[0].x,edge[0].y,cam),startB=worldToScreen(edge[1].x,edge[1].y,cam);let dirAx=startA.x-sourceSx,dirAy=startA.y-sourceSy,dirBx=startB.x-sourceSx,dirBy=startB.y-sourceSy;const lenA=Math.hypot(dirAx,dirAy)||1,lenB=Math.hypot(dirBx,dirBy)||1;dirAx/=lenA;dirAy/=lenA;dirBx/=lenB;dirBy/=lenB;const shadowLen=Math.max(baseLen*1.34,Math.min(baseLen*2.22,dist*1.22+rectR*3.05+188));const farA={x:startA.x+dirAx*shadowLen,y:startA.y+dirAy*shadowLen};const farB={x:startB.x+dirBx*shadowLen,y:startB.y+dirBy*shadowLen};const midX=(startA.x+startB.x)*0.5,midY=(startA.y+startB.y)*0.5,farMidX=(farA.x+farB.x)*0.5,farMidY=(farA.y+farB.y)*0.5;const grad=tctx.createLinearGradient(midX,midY,farMidX,farMidY);grad.addColorStop(0,'rgba(0,0,0,0.84)');grad.addColorStop(0.34,'rgba(0,0,0,0.68)');grad.addColorStop(0.74,'rgba(0,0,0,0.34)');grad.addColorStop(1,'rgba(0,0,0,0.10)');tctx.fillStyle=grad;tctx.beginPath();tctx.moveTo(startA.x,startA.y);tctx.lineTo(startB.x,startB.y);tctx.lineTo(farB.x,farB.y);tctx.lineTo(farA.x,farA.y);tctx.closePath();tctx.fill();}}
function applyZombieBodyShadows(tctx,cam,lightSources){return;}
function carveLocalFlashlightOcclusion(maskCtx,cam,sx,sy,beamAng,opts){const length=opts&&opts.length||560,farW=opts&&opts.farW||230,halfAngle=Math.atan2((farW*0.76*1.28),Math.max(1,length)),maxDist=length+220,baseShadowLen=length*1.26,margin=0.72,sourceWorldX=opts&&Number.isFinite(opts.sourceWorldX)?opts.sourceWorldX:player.x,sourceWorldY=opts&&Number.isFinite(opts.sourceWorldY)?opts.sourceWorldY:player.y;maskCtx.fillStyle='rgba(0,0,0,1)';const insideBuilding=(opts&&opts.insideBuilding)||findContainingBuilding(sourceWorldX,sourceWorldY);if(!insideBuilding){for(const b of WORLD.buildings||[]){const roofVisible=(b.roofAlpha||0)>0.58;if(!roofVisible)continue;const cx=b.x+b.w*0.5,cy=b.y+b.h*0.5,dist=Math.hypot(cx-sourceWorldX,cy-sourceWorldY),approxR=Math.hypot(b.w*0.5,b.h*0.5);if(dist>maxDist+approxR)continue;if(!buildingIntersectsBeam(sourceWorldX,sourceWorldY,beamAng,halfAngle,b,1.18))continue;const screenRect=worldToScreen(b.x,b.y,cam);fillRectOccluder(maskCtx,screenRect.x,screenRect.y,b.w,b.h,2.5);fillBuildingProjectedShadow(maskCtx,cam,sourceWorldX,sourceWorldY,beamAng,halfAngle,b,baseShadowLen);}}for(const z of state.zombies||[]){if(!z||z.dead||z.hp<=0)continue;const radius=Math.max(8,Number(z.radius||10)),dx=z.x-sourceWorldX,dy=z.y-sourceWorldY,dist=Math.hypot(dx,dy);if(dist>maxDist+radius)continue;const centerAng=Math.atan2(dy,dx),tangentHalf=Math.asin(Math.min(0.98,(radius+10)/Math.max(dist,radius+10)));if(Math.abs(angleDiff(centerAng,beamAng))>halfAngle+tangentHalf+margin)continue;const zx=Math.round(sx+dx),zy=Math.round(sy+dy);if(dist>Math.max(8,radius*0.18))fillZombieProjectedShadow(maskCtx,z,zx,zy,centerAng,baseShadowLen*1.04);}if(insideBuilding&&!(opts&&opts.skipInteriorClip)){const r=flashlightInteriorRect(cam,insideBuilding,1);if(r.w>0&&r.h>0){maskCtx.save();maskCtx.globalCompositeOperation='destination-in';maskCtx.beginPath();maskCtx.rect(r.x,r.y,r.w,r.h);maskCtx.fill();maskCtx.restore();}}}
  ...
  drawFog = function(cam){
    ...
    if(online.connected && online.started && onlineIsMode()){
      for(const peer of Object.values(online.peers || {})){
        ...
        const maskCtx = ensureSelfLightMaskCanvas();
        maskCtx.clearRect(0,0,SW,SH);
        const peerInsideBuilding = findContainingBuilding(pxv, pyv);
        const clippedPeer = beginFlashlightBuildingClip(maskCtx, cam, peerInsideBuilding, 1);
        carveFlashlight(maskCtx, s.x, s.y, ang, { length: 560, nearW: 62, farW: 240, halo: 96 });
        maskCtx.globalCompositeOperation = 'destination-out';
        carveLocalFlashlightOcclusion(maskCtx, cam, s.x, s.y, ang, { length: 560, farW: 240, sourceWorldX: pxv, sourceWorldY: pyv, insideBuilding: peerInsideBuilding, skipInteriorClip: clippedPeer });
        maskCtx.globalCompositeOperation = 'source-over';
        if(clippedPeer) maskCtx.restore();
        tctx.drawImage(selfLightMaskCanvas, 0, 0);
