(function(){
  ...
  function localFlashlightAimAngle(){ const mobileAimActive = MOBILE_MODE && touchState.aim.active && Math.hypot(touchState.aim.dx,touchState.aim.dy)>12; if(MOBILE_MODE && !mobileAimActive){ if(player.dashTime>0) return player.dashFacing||0; if(player.rocketJumpTime>0) return Math.atan2(player.rocketJumpVY||0,player.rocketJumpVX||player.faceDir||1); if(player.knockbackTime>0) return Math.atan2(player.knockbackVY||0,player.knockbackVX||player.faceDir||1); if(Math.hypot(touchState.move.dx,touchState.move.dy)>6) return Math.atan2(touchState.move.dy,touchState.move.dx); return (player.faceDir||1)<0?Math.PI:0; } const camAim=camera(); return Math.atan2((camAim.y+state.mouse.y)-player.y,(camAim.x+state.mouse.x)-player.x); }
  function remoteFlashlightAimAngle(peer){ if(!peer) return 0; if((peer.dashTime||0)>0) return Number.isFinite(peer.dashFacing) ? peer.dashFacing : Math.atan2(peer.dashVY||0, peer.dashVX||peer.faceDir||1); if((peer.rocketJumpTime||0)>0) return Math.atan2(peer.rocketJumpVY||0,peer.rocketJumpVX||peer.faceDir||1); if((peer.knockbackTime||0)>0) return Math.atan2(peer.knockbackVY||0,peer.knockbackVX||peer.faceDir||1); if(Number.isFinite(peer.aimAngle)) return peer.aimAngle; return (peer.faceDir||1)<0 ? Math.PI : 0; }
  function drawFlashlightMask(worldX, worldY, angle, cam, isPrimary){ const s=worldToScreen(worldX,worldY,cam), sx=s.x, sy=s.y; ctx.save(); ctx.translate(sx,sy); ctx.rotate(angle||0); const beamLen=isPrimary?300:280, coreW=isPrimary?86:78, outerW=isPrimary?128:118; const nearGlow=ctx.createRadialGradient(0,0,14,0,0,isPrimary?76:68); nearGlow.addColorStop(0,'rgba(0,0,0,1)'); nearGlow.addColorStop(0.55,'rgba(0,0,0,0.84)'); nearGlow.addColorStop(1,'rgba(0,0,0,0)'); ctx.fillStyle=nearGlow; ctx.beginPath(); ctx.arc(0,0,isPrimary?76:68,0,Math.PI*2); ctx.fill(); const outerGrad=ctx.createLinearGradient(-18,0,beamLen,0); outerGrad.addColorStop(0,'rgba(0,0,0,0.92)'); outerGrad.addColorStop(0.35,'rgba(0,0,0,0.80)'); outerGrad.addColorStop(0.82,'rgba(0,0,0,0.42)'); outerGrad.addColorStop(1,'rgba(0,0,0,0.0)'); ctx.fillStyle=outerGrad; ctx.fillRect(-18,-outerW/2,beamLen+18,outerW); const coreGrad=ctx.createLinearGradient(-6,0,beamLen-28,0); coreGrad.addColorStop(0,'rgba(0,0,0,1)'); coreGrad.addColorStop(0.72,'rgba(0,0,0,0.86)'); coreGrad.addColorStop(1,'rgba(0,0,0,0.15)'); ctx.fillStyle=coreGrad; ctx.fillRect(-6,-coreW/2,beamLen,coreW); ctx.fillStyle='rgba(0,0,0,0.34)'; ctx.fillRect(beamLen-34,-coreW*0.34,26,coreW*0.68); ctx.restore(); }
  function flashlightSourcesForCurrentView(){ const out=[]; const onlineActive=online.connected && online.started && onlineIsMode(); if(!onlineActive){ if(!state.running || state.gameOver) return out; out.push({x:player.x,y:player.y,angle:localFlashlightAimAngle(),primary:true}); return out; } if(!online.spectating && !player.dead){ out.push({x:player.x,y:player.y,angle:localFlashlightAimAngle(),primary:true}); } for(const peer of Object.values(online.peers)){ if(!peer || peer.dead) continue; const px=Number.isFinite(peer.displayX)?peer.displayX:(peer.x||0); const py=Number.isFinite(peer.displayY)?peer.displayY:(peer.y||0); out.push({x:px,y:py,angle:remoteFlashlightAimAngle(peer),primary:false}); } return out; }
  const __origDrawFog = drawFog;
  drawFog = function(cam){ const lights=flashlightSourcesForCurrentView(); if(!lights.length){ return __origDrawFog(cam); } ctx.save(); ctx.fillStyle='rgba(0,0,0,0.80)'; ctx.fillRect(0,0,SW,SH); ctx.globalCompositeOperation='destination-out'; for(const light of lights) drawFlashlightMask(light.x, light.y, light.angle, cam, !!light.primary); ctx.globalCompositeOperation='source-over'; const vignette=ctx.createRadialGradient(SW/2,SH/2,Math.min(SW,SH)*0.14,SW/2,SH/2,Math.max(SW,SH)*0.88); vignette.addColorStop(0,'rgba(0,0,0,0)'); vignette.addColorStop(0.72,'rgba(0,0,0,0.10)'); vignette.addColorStop(1,'rgba(0,0,0,0.46)'); ctx.fillStyle=vignette; ctx.fillRect(0,0,SW,SH); ctx.restore(); }
